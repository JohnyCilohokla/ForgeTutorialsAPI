...
public class Base64 {
	...
	static {
		for (int i = 0; i < Base64.map2.length; i++) {
			Base64.map2[i] = -1;
		}
		for (int i = 0; i < 64; i++) {
	...
	public static String encodeLines(byte[] in) {
		return Base64.encodeLines(in, 0, in.length, 76, Base64.systemLineSeparator);
	}
	...
	public static String encodeLines(byte[] in, int iOff, int iLen, int lineLen, String lineSeparator) {
		...
		while (ip < iLen) {
			...
			buf.append(Base64.encode(in, iOff + ip, l));
	...
	public static char[] encode(byte[] in, int iLen) {
		return Base64.encode(in, 0, iLen);
	}
	...
	public static char[] encode(byte[] in, int iOff, int iLen) {
		...
		while (ip < iEnd) {
			...
			int o3 = i2 & 0x3F;
			out[op++] = Base64.map1[o0];
			out[op++] = Base64.map1[o1];
			out[op] = op < oDataLen ? Base64.map1[o2] : '=';
			op++;
			out[op] = op < oDataLen ? Base64.map1[o3] : '=';
			op++;
		}
		return out;
	}
	...
	public static byte[] decode(char[] in, int iOff, int iLen) {
		...
		while (ip < iEnd) {
			...
			int b0 = Base64.map2[i0];
			int b1 = Base64.map2[i1];
			int b2 = Base64.map2[i2];
			int b3 = Base64.map2[i3];
